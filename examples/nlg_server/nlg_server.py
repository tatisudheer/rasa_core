...
async def generate_response(nlg_call, domain):
    ...
    request_id = nlg_call.get("tracker", {}).get("request_id")
    user_id = nlg_call.get("tracker", {}).get("user_id")
    events = nlg_call.get("tracker", {}).get("events")
    tracker = DialogueStateTracker.from_dict(sender_id, request_id, user_id, events, domain.slots)
