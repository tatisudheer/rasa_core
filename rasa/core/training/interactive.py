...
async def send_message(
    ...
    payload = {
        "request_id": DEFAULT_INTERACTIVE,
        "user_id": DEFAULT_INTERACTIVE,
...
async def request_prediction(
    ...
    return await endpoint.request(
        method="post", subpath="/conversations/{}/predict?request_id={}&user_id={}".format(sender_id, request_id, user_id)
    )
...
async def retrieve_tracker(
    ...
    path = "/conversations/{}/tracker?include_events={}&request_id={}&user_id={}".format(
        sender_id, verbosity.name, request_id, user_id
    )
...
async def send_action(
    ...
    subpath = "/conversations/{}/execute?request_id={}&user_id={}".format(sender_id, request_id, user_id)
...
async def send_event(
    ...
    subpath = "/conversations/{}/tracker/events?request_id={}&user_id={}".format(sender_id, request_id, user_id)
...
async def _print_history(sender_id: Text, endpoint: EndpointConfig) -> None:
    ...
    table = _chat_history_table(events, sender_id)
...
def _chat_history_table(events: List[Dict[Text, Any]], sender_id: Text) -> Text:
    ...
    tracker = DialogueStateTracker.from_dict(sender_id, DEFAULT_INTERACTIVE, DEFAULT_INTERACTIVE, events)
