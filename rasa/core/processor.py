...
class MessageProcessor(object):
    ...
    def predict_next(self, sender_id: Text, request_id: Text, user_id: Text) -> Optional[Dict[Text, Any]]:

        # we have a Tracker instance for each user
        # which maintains conversation state
        tracker = self._get_tracker(sender_id, request_id, user_id)
        if not tracker:
            logger.warning(
    ...
    async def log_message(self, message: UserMessage) -> Optional[DialogueStateTracker]:
        ...
        # we have a Tracker instance for each user
        # which maintains conversation state
        tracker = self._get_tracker(message.sender_id, message.request_id, message.user_id)
        if tracker:
    ...
    async def handle_reminder(
        ...
        if (
            ...
            if should_continue:
                user_msg = UserMessage(None, output_channel, sender_id, request_id, user_id)
    ...
    async def _parse_message(self, message):    
        ...
        if str(message.request_id).lower() != 'default':
            logger.debug("Request ID - '{}'".format(message.request_id))
        else:
            logger.warning('No Request ID received by the agent. Invalid Request.')

        if str(message.user_id).lower() != 'default':
            logger.debug("User ID - '{}'".format(message.user_id))
        else:
            logger.warning('No User ID received by the agent. Invalid Request.')
    ...
    @staticmethod
    async def _send_bot_messages(self,
        events: List[Event],
        ...
        for e in events:
            ...
            await output_channel.send_response(tracker.sender_id, tracker.request_id, tracker.user_id, e.message())

        parse_data=self.parse_data
        await output_channel.send_response(tracker.sender_id, tracker.request_id, tracker.user_id, parse_data)
    ...
    async def _run_action(
        ...
        await self._send_bot_messages(self, events, tracker, output_channel)
    ...
    def _get_tracker(self, sender_id: Text, request_id: Text, user_id: Text) -> Optional[DialogueStateTracker]:
        sender_id = sender_id or UserMessage.DEFAULT_SENDER_ID
        return self.tracker_store.get_or_create_tracker(sender_id, request_id, user_id)
