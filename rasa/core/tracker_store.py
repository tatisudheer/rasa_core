...
class TrackerStore(object):
    ...
    def get_or_create_tracker(self, sender_id, request_id, user_id, max_event_history=None):
        tracker = self.retrieve(sender_id, request_id, user_id)
        self.max_event_history = max_event_history
        if tracker is None:
            tracker = self.create_tracker(sender_id, request_id, user_id)
        return tracker

    def init_tracker(self, sender_id, request_id, user_id):
        if self.domain:
            return DialogueStateTracker(
                sender_id, request_id, user_id, self.domain.slots, max_event_history=self.max_event_history
            )
        else:
            return None

    def create_tracker(self, sender_id, request_id, user_id, append_action_listen=True):
        """Creates a new tracker for the sender_id.

        The tracker is initially listening."""

        tracker = self.init_tracker(sender_id, request_id, user_id)
        if tracker:
    ...
    def retrieve(self, sender_id: Text, request_id: Text, user_id: Text) -> Optional[DialogueStateTracker]:
        raise NotImplementedError()

    def stream_events(self, tracker: DialogueStateTracker) -> None:
        offset = self.number_of_existing_events(tracker.sender_id, tracker.request_id, tracker.user_id)
        evts = tracker.events
        for evt in list(itertools.islice(evts, offset, len(evts))):
            body = {"sender_id": tracker.sender_id, "request_id": tracker.request_id, "user_id": tracker.user_id}
            body.update(evt.as_dict())
            self.event_broker.publish(body)

    def number_of_existing_events(self, sender_id: Text, request_id: Text, user_id: Text) -> int:
        """Return number of stored events for a given sender id."""
        old_tracker = self.retrieve(sender_id, request_id, user_id)
    ...
    def deserialise_tracker(self, sender_id, request_id, user_id, _json) -> Optional[DialogueStateTracker]:
...
class InMemoryTrackerStore(TrackerStore):
    ...
    def retrieve(self, sender_id: Text, request_id: Text, user_id: Text) -> Optional[DialogueStateTracker]:
        if sender_id in self.store:
            ...
            return self.deserialise_tracker(sender_id, request_id, user_id, self.store[sender_id])
        else:
...
class RedisTrackerStore(TrackerStore):
    ...
    def retrieve(self, sender_id, request_id, user_id):
        stored = self.red.get(sender_id)
        if stored is not None:
            return self.deserialise_tracker(sender_id, request_id, user_id, stored)
        else:
            return None
...
class SQLTrackerStore(TrackerStore):
    ...
    def _additional_events(self, tracker: DialogueStateTracker) -> Iterator:
        ...
        n_events = self.number_of_existing_events(tracker.sender_id, tracker.request_id, tracker.user_id)
